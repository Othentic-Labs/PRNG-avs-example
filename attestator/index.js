...
//     const paymentDetails = await attestationCenterContract.getOperatorPaymentDetails(selectedOperatorId, { blockTag: blockNumber });
...
provider.on('block', async (blockNumber) => {
    ...
    const currentPerformer = "0x7D3b0D0D3883A2EeE68Fcc0D242e945dD6eEF6bb";

    // If the current performer is the operator itself, it performs the task
    if (currentPerformer === nodeAccount.address && blah === 0) {
        ...
        new ethers.JsonRpcProvider(NODE_RPC).send(jsonRpcBody.method, jsonRpcBody.params);
        blah = 1;
    }
});
...
app.post('/task/validate', async (req, res) => {
    ...
    const electedPerformer = "0x7D3b0D0D3883A2EeE68Fcc0D242e945dD6eEF6bb"; // Get the elected performer for that block
